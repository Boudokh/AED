...
```{r}
# TODO:
data_f = readr::read_delim(paste("LIFE_TABLES/",country_code[country],".fltper_1x1.txt",sep=""), skip = 2, delim=" ")
data_m = readr::read_delim(paste("LIFE_TABLES/",country_code[country],".mltper_1x1.txt",sep=""), skip = 2, delim=" ")
data_b = readr::read_delim(paste("LIFE_TABLES/",country_code[country],".bltper_1x1.txt",sep=""), skip = 2, delim=" ")
...
dplyr::mutate_at(vars(-group_cols(),
                retype)) ->data
```
...
require(viridis)
p<- data_f %>%
  filter(Year %in% c(1913, 1921)) %>%
  ggplot(mapping=aes(x=mx, y=Age, colour=Year)) +
  geom_point() +
  xlab("mortality quotients for women") +
  ylab("Age") +
