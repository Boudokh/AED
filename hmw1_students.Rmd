...
big_data %>%
  filter(Year %in% ww_I) %>%
  ggplot(mapping=aes(x=Age, y=mx)) +
  geom_point(aes(colour = Year), shape = '+', size = 2) +
  scale_x_discrete(breaks = big_data$Age[seq(1,111, by = 10)]) +
  scale_y_log10() +
  facet_wrap(vars(Sex)) +
  xlab("Age")+
  labs(colour = "Sex")
...
age =1
mort = 1
for (i in age:length(mx)){
  exp_a = exp_a + i*mort*mx[i]
  mort = mort*(1-mx[i])
}
...
names(e_f) <- colnames(E_f)

e_f %>% 
  rownames_to_column(var = "Year") -> e_f
...
e_f %>%
  ...
  geom_point(aes(colour = Age)) + 
  scale_y_log10() +
  scale_x_discrete(breaks = e_f$Year[seq(5,202, by = 50)]) +
  facet_wrap(~ Age)
