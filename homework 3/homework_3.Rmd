...
```{r}
years = c(1933:1995)
lee_carter = function(df,ref_country,years)
{
  lt <- df %>%
  select(Year, Age, Gender,Country, mx) %>%
  tidyr::pivot_wider(names_from = "Age", values_from = "mx")
  lt%>%
    filter(Country == ref_country, Year %in% years) -> lt
  ...
  for (i in 1:length(k))
  {
    mx_lc = append(mx_lc,exp(a + b*k[i]))
  }
  result <- filter(df, Year %in% years, Country == ref_country) %>%
...
df_lc = df %>%
  select(Year,Age,Gender,Country,mx)
usa_lc_33_95 = lee_carter(df_lc,"USA",years)
years = c(2000:2015)
usa_lc_00_15 = lee_carter(df_lc,"USA",years)
p<- ggplot() + 
  geom_point(aes(Age, mx, color=Year)) + 
  geom_line(aes(Age,mx_lc,color=Year)) + 
  scale_y_log10()+
  facet_wrap(~Gender) 
years = seq(2000,2015,by = 5)
(p%+%
  filter(usa_lc_00_15,Year %in% years))%>%ggplotly()
```
