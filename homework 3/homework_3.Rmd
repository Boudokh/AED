...
#   glimpse(data_b)
#   sapply(data_b, class)
full_data <- ldply(out, data.frame) 
...
ratio_us_1948 <- function(tab){
  tab  %>%
    filter(Gender == 'Both' ) %>%
    mutate(ratio_us = mx/data_b_us$mx) %>%
    filter(Year == 1948) -> tab
  return(tab)
}
...
p <- out_us %>%
  ...
  ylab("Death rates") +
  labs(colour = "Countries") +
  ggtitle("Ratios between central death rates in European countries and
central death rates in the USA in 1948") +
  theme_bw()
...
for (df in out){
  df %>%
    filter(Gender == 'Both') %>%
    mutate(ratio_us = data_b_us$qx/qx) %>%
    filter(Year >= 1933) %>%
    print()
}
...
p <- full_data %>%
  filter(Year %in% after_ww_II, Gender != 'Both') %>%
  ggplot(mapping=aes(x=Age, y=mx)) +
...
ratio_mortality_rates <- function(df,
  ...
  dplyr::inner_join(y=df[df$Year==1946, c("Age", "mx","Country")], by=c("Age","Country")) %>%
...
```{r}
q <- dplyr::filter(ratio_mortality_rates(full_data), FALSE)%>%
...
```{r}
netherlands_male <- full_data %>%
  dplyr::filter(Country == 'Netherlands',Gender == 'Male') 
(q  %+%
  ratio_mortality_rates(netherlands_male) +
  ggtitle("Variation of mortality quotient with respect to Y=1946, Males in Netherlands")) %>%
  ggplotly()
```

```{r}
netherlands_female <- full_data %>%
  dplyr::filter(Country == 'Netherlands',Gender == 'Female') 
(q  %+%
  ratio_mortality_rates(netherlands_female) +
  ggtitle("Variation of mortality quotient with respect to Y=1946, Females in Netherlands")) %>%
  ggplotly()
```
...
half_full_data_twogenders <- full_data %>%
  dplyr::filter(Country %in% c('Netherlands','USA'),Gender != 'Both')
ages <- c(0, 1, 5)

mx_by_age <- half_full_data_twogenders %>%
  filter(Age %in% ages, Gender != 'Both') %>%
  ggplot(mapping=aes(x=Year, y=mx)) +
  geom_line(size = 0.1, aes(colour =  forcats::as_factor(Age))) +
  facet_grid(vars(Country), vars(Gender)) +
  scale_y_log10() +
  ylab("Mortality quotients")+
  labs(color = 'Age') +
  ggtitle("Variation of mortality quotient at ages 0, 1 and 5 as a function of time") +
  theme_bw()
...
mx_by_age_2 <- half_full_data_twogenders %>%
  filter(Age %in% ages_2, Gender != 'Both') %>%
  ggplot(mapping=aes(x=Year, y=mx)) +
  geom_line(size = 0.1, aes(colour =  forcats::as_factor(Age))) +
  facet_grid(vars(Country), vars(Gender)) +
  scale_y_log10() +
...
life_table_pivot_log <- full_data %>%
  select(Country, Gender, Year, Age, mx) %>%
...
re %>%
  ...
  facet_wrap(~ Age) +
  geom_point(shape="+") +
  ggtitle('Residual life expectancy as a function of Year at ages 60 and 65') +
  theme_bw()
...
```{r}
pca_data <- out_pivot_log$data_f_nl
pca_data[unlist(lapply(1948:2010, as.character)),] ->pca_data
out_rex$data_b_nl[unlist(lapply(1948:2010, as.character)),1] -> rx
pca_data <- cbind(pca_data, rx)
res.pca = prcomp(pca_data, scale. = T, center = T)
