...
Lee_carter_fit<- function(df, c, g, y_start, y_lc ,y_end){
  ...
  for (i in 1:(y_end-y_lc)){
    eps  = rnorm(1, mean = 0, sd = sqrt(sd(k_t)))
    k_t <- cbind(k_t,k_t[length(k_t)]+drift+eps)
  }
...
lt_lc <- Lee_carter_fit(life_table_pivot_log, 'USA', 'Female', 1933, 1995, 2040)
...
lt_lc_us <- merge(data_b_us, lt_lc_us)
lt_lc_us <- lt_lc_us[order(lt_lc_us$Year, lt_lc_us$Age),]
