...
```{r}
ratio_us_1948 <- function(tab, year){
  us_48 <- data_b_us %>%
...
out_us <- purrr::map(out, ratio_us_1948, 1999)
...
plot_pca<-function(data, sca, cen, c, g){
  pca_data <- data %>%
  ...
  filter(life_table_pivot, Country == c, Year %in% 1948:2010, Gender == g)$life_ex -> rx
...
plot_pca(life_table_pivot_log, TRUE, TRUE, 'Netherlands', 'Both')
#plot_pca(out_pivot_log$data_f_nl, FALSE, TRUE, 'Netherlands', 'Both')
#plot_pca(out_pivot_log$data_f_nl, TRUE, FALSE, 'Netherlands', 'Both')
#plot_pca(out_pivot_log$data_f_nl, FALSE, FALSE, 'Netherlands', 'Both')
```
